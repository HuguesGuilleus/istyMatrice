// https://github.com/clibs/flag/blob/master/flag.c
...
#define FLAG_TYPE(NAME, TYPE, CAP)                                             \
	void flagset_##NAME(flagset_t *self, TYPE *value, const char *name,           \
	                    const char *help) {                                       \
		CHECK_BOUNDS;                                                                \
		self->flags[self->nflags++] = (flag_t){                                      \
		    .name = name, .help = help, .type = FLAG_TYPE_##CAP, .value = value};    \
	}                                                                             \
	void flag_##NAME(TYPE *value, const char *name, const char *help) {           \
		if (set == NULL)                                                             \
			set = flagset_new();                                                        \
...
static inline bool is_flag(const char *s) {
...
static int largest_flag_name(flagset_t *self) {
	int max = 0;
...
void flagset_free(flagset_t *self) { free(self); }

/*
 * Find flag matching `arg` --no-`arg`.
 */

static flag_t *flagset_find(flagset_t *self, const char *arg) {
	for (int i = 0; i < self->nflags; ++i) {
		flag_t *flag = &self->flags[i];
